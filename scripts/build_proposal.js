  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({ spacing: { after: 200 },
        children: [new TextRun({ text: "& a Proactive Sales-Insight Agent", bold: true, size: 40, color: NAVY, font: FONT })] }),
      ...
      P([T("Reference       ", { bold: true, color: NAVY }), T("TS-2026-07 · Valid for 30 days")], { after: 240 }),
      new Table({
        columnWidths: [CW],
        ...
        rows: [ new TableRow({ children: [ new TableCell({
          ...
          children: [
            ...
            new Paragraph({ children: [ new TextRun({ text: "Management accounts that write themselves — a clean monthly, quarterly and annual report produced automatically from your sales data, plus an agent that watches the numbers and flags what needs your attention, so neither of you has to remember to sit down and do the books.", size: 22, color: NAVY, font: FONT }) ] }),
          ],
        }) ] }) ],
      }),
      ...
      P("Thread & Salt is a healthy, growing business — but the numbers that would tell you that are locked away. The books live in Xero, sales sit in a Google Sheet, and management reporting only happens when one of you remembers, which by your own account is rarely. The result is a profitable company run largely on instinct, with no regular, readable view of how it is actually doing."),
      P([T("This proposal fixes that in two moves. "), T("First (the core): ", { bold: true, color: NAVY }), T("we build one polished management-accounts report end to end from your real sales data, get your sign-off on it, then make it run itself on a monthly, quarterly and annual cadence — no founder input required. "), T("Second (the stretch): ", { bold: true, color: NAVY }), T("we add a proactive agent that reads each new month, spots trends and at-risk lines, and tells you when to restock, discount or promote — before you would have thought to look.")]),
      P([T("The whole engagement runs for three weeks, with a working report in your hands inside the first week. Total investment is "), T("£4,400 fixed", { bold: true, color: NAVY }), T(", with a core-only option at "), T("£3,200", { bold: true, color: NAVY }), T(" — both inside the budget you set. Everything is built to be, in your words, dead simple to use: you open a document, you don't operate software.")]),
      ...
      bullet([T("The data is real, and a bit messy. ", { bold: true }), T("Twelve-plus months of orders live in a Google Sheet with inconsistent columns and the odd blank cell. Cleaning it up is part of the job, not an afterthought.")]),
      bullet([T("Xero holds the truth, but nobody looks. ", { bold: true }), T("The real financials are in Xero; today they go unread. We can work from the sheet, from Xero, or reconcile both.")]),
      bullet([T("It has to be effortless. ", { bold: true }), T("You are a two-person shop. Realistically you will only use something if it is dead simple — so the finished thing must require essentially nothing from you month to month.")]),
      ...
      num([T("Clean the data once, properly. ", { bold: true }), T("We take your Google Sheet / CSV export, standardise the columns, resolve blanks and inconsistencies, and produce a single reliable dataset the reports are built from. (If you prefer, we connect Xero directly instead — decided at kickoff.)")], "steps"),
      num([T("Nail one report, end to end. ", { bold: true }), T("We build one management-accounts report from your real numbers — a P&L-style summary with revenue by month, quarter and year, gross and net margin, an operating-cost breakdown, top-selling lines and a channel split — in a clean, readable format you sign off on. (A worked sample sits alongside this proposal.)")], "steps"),
      num([T("Make the cadence run itself. ", { bold: true }), T("We schedule the report to regenerate automatically — monthly, quarterly and annually — in the same shape every time, and land in your inbox / shared folder. No one has to remember, and no one has to press a button.")], "steps"),
      num([T("Make it dead simple. ", { bold: true }), T("You receive a finished, formatted document you can read on a phone. We include a one-page plain-English guide and a short walkthrough so both founders know how it works.")], "steps"),
      spacer(40),
      ...
      bullet([T("Watches every new month of data automatically as it lands.")]),
      bullet([T("Surfaces genuine, actionable signals: a hero line that is quietly declining, a product due a restock, a channel pulling ahead, an unusual dip worth a second look.")]),
      bullet([T("Delivers a short, prioritised note in plain English — “here is what changed and here is what we'd do about it” — attached to the monthly report.")]),
      bullet([T("Improves over time as more months accumulate, without extra work from you.")]),

      // ---------- 4. OUT OF SCOPE ----------
      H1("4.  Assumptions & what's out of scope"),
      P("To keep the fixed price honest and the timeline firm, this engagement assumes:"),
      bullet([T("Data access is provided at kickoff — the Google Sheet / CSV, and Xero sandbox credentials if we take the Xero route.")]),
      bullet([T("Reports are built on the data you provide; we reconcile and flag anomalies but do not act as your accountant or file anything with HMRC.")]),
      bullet([T("Financial figures are management information for internal decisions, not audited or statutory accounts.")]),
      bullet([T("The following are out of scope for this engagement and can be quoted separately if wanted: bookkeeping or data entry into Xero, VAT / tax filing, inventory-system integration, a live web dashboard, and forecasting/budgeting models.")]),
      ...
      bullet([T("Reading & cleaning your data ", { bold: true }), T("directly from the sheet or CSV, turning a messy export into a tidy, trustworthy dataset.")]),
      bullet([T("Generating the report ", { bold: true }), T("as a polished spreadsheet and document, in a fixed structure so every month looks identical and comparable.")]),
      bullet([T("Scheduled routines ", { bold: true }), T("that trigger the monthly, quarterly and annual runs on their own, with nobody in the loop.")]),
      bullet([T("A connector to Xero ", { bold: true }), T("if we go that route, so the numbers come straight from source.")]),
      P("Everything runs on your data, kept private to your business, and the outputs are ordinary documents you already know how to open."),
      ...
      table([1520, 3040, 3560, 1240], [
        ...
        new TableRow({ children: [
          ...
          cell("Discovery, data access, clean the dataset, build one report end-to-end", { w: 3560, fill: LIGHT }),
        ...
        new TableRow({ children: [
          ...
          cell("Automate the monthly / quarterly / annual cadence; founder training & guide", { w: 3560 }),
      ...
      spacer(60),
      P([T("Hard cut-off. ", { bold: true, color: NAVY }), T("Delivery and handover complete "), T("Friday 15 August 2026", { bold: true }), T(". A two-week window for fixing any defects runs to "), T("Friday 29 August 2026", { bold: true }), T(", at which point the engagement is formally closed. Any new work requested after that date is a separate, separately-quoted engagement. This keeps scope tight and the fixed price fair to both sides.")]),
      ...
      table([4360, 2560, 2440], [
        ...
        new TableRow({ children: [
          cell([new Paragraph({ children:[new TextRun({text:"Core — Management accounts on autopilot",bold:true,size:20,font:FONT,color:NAVY})]}),
                new Paragraph({ spacing:{before:30}, children:[new TextRun({text:"Data cleaning, one signed-off report, automated monthly/quarterly/annual cadence, guide & training.",size:18,font:FONT,color:GREY})]})], { w: 4360, fill: LIGHT }),
      ...
      P([T("Our time. ", { bold: true, color: NAVY }), T("This is roughly six focused working days of our effort across the three weeks. We quote it as a fixed fee rather than by the hour so you carry no risk if something takes us longer than planned — that's on us, not you.")]),
      ...
      num([T("Access to the sales Google Sheet / CSV, and to a Xero sandbox if we take that route (we'll confirm which at kickoff).")], "need"),
      ...
      bullet([T("SKU-level profitability once product costs are mapped, so you see margin by line, not just revenue.")]),
      ...
      spacer(120),
      table([3000, 3180, 3180], [
        new TableRow({ children: [
          cell("Selected package", { w: 3000, bold: true, color: NAVY }),
          cell("□  Core (£3,200)      □  Core + Stretch (£4,400)", { w: 6360, size: 20 }),
        ]}),
      ]),
      spacer(160),
      table([4680, 4680], [
...
Packer.toBuffer(doc).then(b => {
  fs.writeFileSync("Thread-and-Salt-Proposal.docx", b);
  console.log("wrote Thread-and-Salt-Proposal.docx", b.length, "bytes");
});
